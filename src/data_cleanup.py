...
def cleanup(match):
    ...
    # load item map-array
    item_map = np.load("SRitems.npy").tolist()
    ...
    for participant in participants:
        ...
        for k in participant:
            ...
            if k == 'stats':
                # prepare item block
                if 'items' not in d:
                    d['items'] = [None] * NUM_PLAYERS
                d['items'][playerId - 1] = [0]*len(item_map)
                stats = participant[k]
                for k_stat in stats:
                    # Ignore some of the keys like "winner"
                    if k_stat == 'winner':
                        continue
                    # if valid item, show we had item
                    if k_stat[:4] == 'item':
                        if stats[k_stat] in item_map:
                            d['items'][playerId - 1][item_map.index(stats[k_stat])] = 1
                    if k_stat not in d:
